...
export async function deleteCountry(id) {
  ...
  if (isStatic) {
    ...
    if (!deletedIds.includes(id)) { deletedIds.push(id); await localforage.setItem('deleted_countries', deletedIds); }
  }
  let countries = await localforage.getItem('countries') || [];
  if (countries.length > 0) { countries = countries.filter(c => c.id !== id); await localforage.setItem('countries', countries); }
...
export async function updateCountry(id, newName) {
  ...
  if (index !== -1) { countries[index].name = newName; await localforage.setItem('countries', countries); }
}
...
export async function getGroups(countryId) {
  ...
  if (localGroups.length > 0) { return localGroups.filter(g => !deletedIds.includes(g.id)); }
...
export async function deleteGroup(countryId, groupId) {
  ...
  if (isStatic) {
    const deletedIds = await localforage.getItem(`deleted_groups_${countryId}`) || [];
    if (!deletedIds.includes(groupId)) { deletedIds.push(groupId); await localforage.setItem(`deleted_groups_${countryId}`, deletedIds); }
  }
  let localGroups = await localforage.getItem(`groups_${countryId}`) || [];
  if (localGroups.length > 0) { localGroups = localGroups.filter(g => g.id !== groupId); await localforage.setItem(`groups_${countryId}`, localGroups); }
}

export async function updateGroup(countryId, groupId, newName) {
  let groups = await getGroups(countryId);
  const index = groups.findIndex(g => g.id === groupId);
  if (index !== -1) { groups[index].name = newName; await localforage.setItem(`groups_${countryId}`, groups); }
}
...
export async function deleteImage(countryId, imageId) {
  ...
  if (isStatic) {
    const deletedIds = await localforage.getItem(`deleted_images_${countryId}`) || [];
    if (!deletedIds.includes(imageId)) { deletedIds.push(imageId); await localforage.setItem(`deleted_images_${countryId}`, deletedIds); }
  }
  let localImages = await localforage.getItem(`images_${countryId}`) || [];
  if (localImages.length > 0) { localImages = localImages.filter(img => img.id !== imageId); await localforage.setItem(`images_${countryId}`, localImages); }
}
...
export async function updateImage(countryId, imageId, updates) {
  ...
  if (index !== -1) { images[index] = { ...images[index], ...updates }; await localforage.setItem(`images_${countryId}`, images); }
}
